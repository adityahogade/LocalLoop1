...
module.exports = {
  create: Joi.object({
    ...
    service_plan_id: id.required(),
    address_id: id.required(),
    quantity: Joi.number().positive().max(1000).required(),
    delivery_time_slot: Joi.string().valid("morning", "evening", "custom").optional().default("morning"),
    custom_time: time.allow(null),
    delivery_slots: Joi.array().items(
      Joi.alternatives().try(
        Joi.string().valid("morning", "evening", "custom"),
        Joi.object({
          slot: Joi.string().valid("morning", "evening", "custom").required(),
          custom_time: time.allow(null, "").optional(),
        }).unknown(true)
      )
    ).optional(),
    first_delivery_choice: Joi.string().valid("today", "tomorrow", "custom").optional(),
